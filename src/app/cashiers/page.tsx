...
export default function CashiersManagementPage() {
  ...
  const filteredStaff = staff.filter(
    (member) =>
      member.name.includes(searchQuery) || member.username.includes(searchQuery)
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            },
            {
              ...
              value: activeStaff,
              sub: `${activeStaff} حساب يستقبل العمل حالياً`,
              icon: UserCheck,
              color: "text-emerald-400",
            },
            {
              title: "حسابات الإدارة / الملاك",
              value: adminManagerStaff,
              sub: "درجة مسؤولين ومدراء نظام",
              icon: ShieldAlert,
              color: "text-amber-400",
            },
          ].map((kpi, idx) => (
            <div key={idx} className="rounded-2xl border border-border/70 bg-card/80 p-4 relative overflow-hidden shadow-sm">
              ...
              <TableBody>
                {filteredStaff.length === 0 ? (
                  ...
                  filteredStaff.map((member) => (
                    <TableRow key={member.id} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                      <TableCell className="font-bold text-slate-200 text-xs py-4">{member.name}</TableCell>
                      <TableCell className="text-slate-400 font-mono text-xs">{member.username}</TableCell>
                      <TableCell>
                        <Badge
                          variant="secondary"
                          className={cn(
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Switch checked={member.active} onCheckedChange={() => handleToggleActive(member.id)} />
                          <span className={cn("text-[10px] font-bold min-w-8", member.active ? "text-emerald-400" : "text-red-400")}>{member.active ? "نشط" : "معطل"}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-center">
              ...
              <div className="space-y-2.5">
                ...
                {(
                  [
                    ...
                    { key: "printerSettings", label: "إعدادات الطباعة", desc: "إدارة الطابعات والفواتير والتقارير المطبوعة" },
                    { key: "messages", label: "سجل الرسائل", desc: "محادثة العملاء وتصفح سجل الدعم الفني" },
                    { key: "staffManagement", label: "إدارة الموظفين والصلاحيات", desc: "إضافة وتعديل صلاحيات زملاء الموظفين" },
                  ] as const
                ).map((perm) => (
